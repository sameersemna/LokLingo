...
class ValidateReliabilityArtifactsTests(unittest.TestCase):
    ...
    def test_rejects_missing_onepager_fallback_signal_when_smoke_has_value(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            smoke = tmp / "smoke.json"
            snapshot = tmp / "snapshot.json"
            ...
            self.assertIn("fallback-budget triage signal", result.stderr)

    def test_accepts_onepager_fallback_signal_when_smoke_has_value(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            smoke = tmp / "smoke.json"
            snapshot = tmp / "snapshot.json"
            ...
            _write(
                ...
                "\n".join(
                    [
                        ...
                        "- Recommended First Command: bash guide/ops/reliability-recovery.sh queue-status",
                        "- Correlation ID: corr-x",
                        "- OCR Fallback Budget Exhausted Signal: total=2, status=warning, guidance=warn delta > 0 and < 3 over 10m, critical delta >= 3 over 10m",
                        "",
                        "## Operator Handoff Commands",
                    ]
                )
