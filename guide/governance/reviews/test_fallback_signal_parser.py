...
class FallbackSignalParserTests(unittest.TestCase):
    ...
    def test_parses_scientific_notation_value(self) -> None:
        lines = [
            "| OCR Fallback Budget Exhausted Total | 1e1 | warn delta > 0 and < 3 over 10m, critical delta >= 3 over 10m |",
        ]

        parsed = parse_fallback_budget_signal(lines)
        self.assertTrue(parsed.found)
        self.assertFalse(parsed.malformed)
        self.assertEqual(parsed.value, "1e1")
