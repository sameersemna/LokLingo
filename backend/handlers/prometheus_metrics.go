...
func (h *PrometheusMetricsHandler) Expose(c *fiber.Ctx) error {
	...
	b.WriteString("# TYPE loklingo_ocr_correction_applied_total counter\n")
	fmt.Fprintf(&b, "loklingo_ocr_correction_applied_total %d\n", reliability.OCR.CorrectionAppliedTotal)
	b.WriteString("# TYPE loklingo_ocr_correction_latency_avg_ms gauge\n")
	fmt.Fprintf(&b, "loklingo_ocr_correction_latency_avg_ms %g\n", reliability.OCR.CorrectionLatencyMS.Avg)
	b.WriteString("# TYPE loklingo_ocr_correction_changed_characters_total counter\n")
	fmt.Fprintf(&b, "loklingo_ocr_correction_changed_characters_total %d\n", reliability.OCR.CorrectionChangedCharactersTotal)
	b.WriteString("# TYPE loklingo_ocr_correction_confidence_delta_avg gauge\n")
	fmt.Fprintf(&b, "loklingo_ocr_correction_confidence_delta_avg %g\n", reliability.OCR.CorrectionConfidenceDelta.Avg)
