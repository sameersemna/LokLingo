...
func TestCreateImageJob_MismatchedExtensionButValidPNGAccepted(t *testing.T) {
	cs := &imageJobStore{}
	app, _ := newImageJobsApp(cs)

	req := newMultipartImageRequest(t, []byte{0x89, 'P', 'N', 'G', '\r', '\n', 0x1a, '\n'}, "photo.webp", "de", "en", "overlay")
	resp, err := app.Test(req)
	if err != nil {
		t.Fatalf("request failed: %v", err)
	}
	if resp.StatusCode != http.StatusAccepted {
		t.Fatalf("expected 202 for mismatched-but-valid image upload, got %d", resp.StatusCode)
	}
	if cs.job == nil {
		t.Fatal("expected job to be enqueued")
	}
