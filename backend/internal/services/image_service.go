...
type overlayCandidate struct {
	...
	regionClass  string // e.g. "title", "heading", "body", "code"
}
...
type ImageTextBlock struct {
	...
	RegionClass string    `json:"region_class,omitempty"` // "title", "heading", "body", "code", "caption", etc.
}
...
func DrawTextOnImageWithOptions(imagePath string, blocks []ImageTextBlock, translatedTexts []string, opts OverlayOptions) (string, OverlayStats, error) {
	...
	for _, candidate := range candidates {
		...
		// Apply region-class font boost for Studio mode.
		...
		// Studio: region class overrides automatic style detection.
		if opts.StudioMode {
...
type OverlayOptions struct {
	...
	// Studio-mode enhancements (only active when StudioMode is true).

	// StudioMode enables the high-quality document reconstruction render path.
	// When true, texture-aware fill, wider feathering, anti-halo cleanup, and
	// adaptive font scaling are all applied. Default: false.
	StudioMode bool
	// TextureAwareFill samples border pixels to synthesise a background texture
	// instead of a flat solid fill, reducing the "painted box" artefact.
	// Requires StudioMode=true. Default: false.
	TextureAwareFill bool
	// AntiHaloRadius erases a ring of extra pixels around each bbox before fill
	// to prevent bright halos on dark/textured backgrounds. 0 = disabled.
	// Recommended: 2-4 px for Studio. Default: 0.
	AntiHaloRadius int
	// TitleFontBoost is an extra font-size bonus applied to blocks classified as
	// region_class="title". Helps headings appear larger than body text.
	// Default: 0.
	TitleFontBoost int
	// HeadingFontBoost is an extra font-size bonus for region_class="heading".
	// Default: 0.
	HeadingFontBoost int
	// CodeFontMono when true forces monospace font for region_class="code" blocks.
	// Default: false.
	CodeFontMono bool
	// RegionClass carries the per-block region classification propagated from
	// the OCR service ("title", "heading", "body", "code", "annotation", etc.).
	// Callers set this per-block before calling the draw loop.
	RegionClass string
}
...
// DefaultStudioOptions returns high-quality rendering options for Studio mode
// (ModeLayout). Compared to DefaultOverlayOptions, Studio mode uses:
//   - Larger max font size for title/heading regions
//   - Wider feathering and blur for smoother patch boundaries
//   - Texture-aware fill to preserve background detail
//   - Anti-halo cleanup to avoid bright rings on dark backgrounds
//   - Higher JPEG quality for premium output
func DefaultStudioOptions() OverlayOptions {
	return OverlayOptions{
		BgAlpha:                   overlayBgAlpha,
		MaxFontSize:               overlayMaxFontSize + 8, // up to 40pt for large headings
		MinFontSize:               overlayMinFontSize,
		JPEGQuality:               95,
		TextPadding:               overlayTextPadding + 2,
		BboxShrinkPx:              1, // gentler shrink — keep more of the OCR geometry
		PatchFeatherPx:            4, // wider feather for smoother patch blend
		PatchBlurRadius:           2, // slightly stronger blur
		EraseBBox:                 true,
		DisableFontStyleDetection: false,
		DisableColorSampling:      false,
		StudioMode:                true,
...
// medianRingColor samples a ring around inner and returns the per-channel median
// colour. It is used by Studio mode to approximate the surrounding texture
// without sampling the OCR text region itself.
...
// In StudioMode the fill region can be expanded by AntiHaloRadius to remove
// bright halo pixels left by text anti-aliasing before the texture fill starts.
func patchFillBBoxWithOptions(dst *image.RGBA, box image.Rectangle, opts OverlayOptions) {
	...
	// Studio anti-halo: expand fill area slightly outward to remove bright halo
	// pixels caused by text anti-aliasing against the background.
	...
	maxFeather := 3
	if opts.StudioMode {
		maxFeather = 8 // allow wider feathering in Studio mode for smoother blends
	}
	...
	// TextureAwareFill: sample from a ring of pixels just outside the fill box
	// to get a better-representative background color for each grid cell,
	// preventing the fill from averaging in the to-be-erased text ink pixels.
	if opts.StudioMode && opts.TextureAwareFill {
		ringWidth := max(2, opts.AntiHaloRadius+1)
		for gy := 0; gy < gridN; gy++ {
			for gx := 0; gx < gridN; gx++ {
				// Sample a slightly expanded cell and ignore the inner text rectangle.
